...
void exam::end_exam()
{
    ...
    {
        if (student)
            tmp = "bash .system/data_sender.sh \"exam_success_end: basic_python_algorithms\"";
        else
            tmp = "bash .system/data_sender.sh \"exam_success_end: basic_python_algorithms\"";
        system(tmp.c_str());
        if (student)
            std::cout << WHITE << BOLD << "🥳 Congratulation! You have finished Basic Python Algorithms!" << std::endl;
        else
            std::cout << WHITE << BOLD << "🥳 Congratulation! You have finished the Exam Week 0" << exam_number << " !" << std::endl;
    }
    else
    {
        std::cout << WHITE << BOLD << "🙁 You have finished Basic Python Algorithms, " << RED << BOLD << "after using " << using_cheatcode << " cheat command..." << WHITE << BOLD << std::endl;
        if (student)
            tmp = "bash .system/data_sender.sh \"exam_success_cheat" + std::to_string(using_cheatcode) + ": basic_python_algorithms\"";
        else
            tmp = "bash .system/data_sender.sh \"exam_success_cheat" + std::to_string(using_cheatcode) + ": basic_python_algorithms\"";
        system(tmp.c_str());
    }
